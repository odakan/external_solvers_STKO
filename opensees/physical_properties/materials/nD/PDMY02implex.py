...
def makeXObjectMetaData():
    ...
    # algorithm
	# choose beetwen IMPLICIT or IMPL-EX solution
	at_algorithm = MpcAttributeMetaData()
	at_algorithm.type = MpcAttributeType.String
	at_algorithm.name = 'integration'
	at_algorithm.group = 'Misc'
	at_algorithm.description = (
		html_par(html_begin()) +
		html_par(html_boldtext('Integration type')+'<br/>') +
		html_par("(Default) Implicit: A standard Backward-Euler integration scheme.<br/>"
	...
	at_algorithm.setSourceList(['Implicit', 'IMPL-EX'])
	at_algorithm.setDefault('Implicit')
    
    # print information
	# choose beetwen silent or verbose mode
	at_info = MpcAttributeMetaData()
	at_info.type = MpcAttributeType.String
	at_info.name = 'information'
	at_info.group = 'Misc'
	at_info.description = (
		html_par(html_begin()) +
		html_par(html_boldtext('Vebosity behavior')+'<br/>') +
		html_par("(Default) Silent: No information about internal processes will be printed.<br/>"
		"Verbose: Some information regarding the material and internal solution steps will be displayed on the terminal. (Useful for debugging)") +
		html_par(html_href('http://opensees.berkeley.edu/wiki/index.php/PressureIndependMultiYield_Material','PressureIndependMultiYield Material')+'<br/>') +
		html_end()
		)
	at_info.sourceType = MpcAttributeSourceType.List
	at_info.setSourceList(['Silent', 'Verbose'])
	at_info.setDefault('Silent')
	
	
	xom = MpcXObjectMetaData()
	xom.name = 'PDMY02implex'
...
def writeTcl(pinfo):
	...
	algorithm_at = xobj.getAttribute('integration')
	if(algorithm_at is None):
		raise Exception('Error: cannot find "integration" attribute')
    
	if (algorithm_at.string == 'IMPL-EX'):
		opt.append('-implex')
        
	info_at = xobj.getAttribute('information')
	if(info_at is None):
		raise Exception('Error: cannot find "information" attribute')
    
	if (info_at.string == 'Verbose'):
		opt.append('-info')
	...
	if Optional:
		...
		if automatic_surface:
			...
			surf += '-{}'.format(r_Gs.rows)
			
			i=0			#rows
			while(i<100 and i<r_Gs.rows):
				j=0		#columns
				while(j<2):
					if(j==0):
						surf+='\\\n\t{}{}'.format(pinfo.indent, r_Gs.valueAt(i,j))
					else:
						surf+=' {}{}'.format(pinfo.indent, r_Gs.valueAt(i,j))
					j+=1
				i+=1
	
	str_tcl = '{}nDMaterial PDMY02Implex {} -rho {} -Gref {} -Kref {} -fAngle {} -gPeak {} -Pref {} -mPow {}\\\n\t-ptAngle {} -c1 {} -c3 {} -d1 {} -d3 {} -tnys {}\\\n\t-c2 {} -d2 {} -l1 {} -l2 {} -e {} -csl1 {} -csl2 {} -csl3 {} -Patm {} -cohesion {} -hv {} -pv {} {}\n'.format(
			pinfo.indent, tag, rho.value, refShearModul.value, refBulkModul.value, frictionAng, peakShearStra, refPress.value, pressDependCoe, 
            PTAng, contrac1, contrac3, dilat1, dilat3, surf, 
            contrac2, dilat2, liquefac1, liquefac2, e, cs1, cs2, cs3, pa.value, c.value, Hv, Pv, opt)
